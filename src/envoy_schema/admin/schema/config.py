...
class RuntimeServerConfigRequest(BaseModel):
    ...
    dcap_pollrate_seconds: int | None = None  # If set - update the DeviceCapability pollRate
    edevl_pollrate_seconds: int | None = None  # If set - update the EndDeviceList pollRate
    fsal_pollrate_seconds: int | None = None  # If set - update the FunctionSetAssignmentList pollRate
    ...
    site_control_pow10_encoding: int | None = None  # If set - update the pow10 encoded for sent DERControl values.

    disable_edev_registration: bool | None = None  # If True - EndDevice RegistrationLink's will be disabled
