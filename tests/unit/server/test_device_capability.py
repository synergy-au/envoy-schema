...
def test_DeviceCapabilityResponse_links():
    ...
    entity = generate_class_instance(DeviceCapabilityResponse, optional_is_none=False, generate_relationships=True)
    assert entity.TimeLink is not None
    assert entity.EndDeviceListLink is not None
    assert entity.MirrorUsagePointListLink is not None
    xml = to_xml(entity)
